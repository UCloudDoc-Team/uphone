...
>1.可尝试使用游戏官网的版本安装，TapTap和应用宝等渠道更改了包的信息，有时会存在兼容问题 
>
>2.可先尝试使用UCloud应用市场里的版本进行安装
>
>3. 反馈给UCloud团队更新应用市场
...
> 
> 4. 行为方面：不要短时间内频繁点赞关注，在inbox里回关他人，注重完播率、观看某用户的多个视频、搜索话题，查看一些shop店铺信息，多做一些未做过的操作

### Q：TikTok获得点击量有什么要注意的？
> 1）新账号注册后不要直接上传视频、不要大量关注点赞（急功近利容易被标记为营销号），冷却几天再像正常用户一样每天点击、上传2-3个优质视频
> 
> 2）目前使用gmail注册过于泛滥，使用手机号注册更容易获得流量，绑定insgram facebook 手机号更佳（当地手机号最佳） 
> 
> 3) 确保一机一号一地域，不要一个账号跨地域使用（人不可能在短时间内出现在另一个国家） 
> 
> 4）最终原则就是像真实个人用户一样的行为，平台鼓励优质视频，建设良好生态！ Good Luck！祝店铺商品大卖！
...
### Q:是否支持自定义安卓ROM、刷机？
> 目前可以基于UCloud标准的安卓镜像安装APP、更新Launcher、更换Logo和安卓机型参数来自定义安卓系统， 不支持源码级别的自定义，不支持将完整的安卓系统二进制刷入云手机。
